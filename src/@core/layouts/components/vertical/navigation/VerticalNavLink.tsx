...
>(({ theme }) => ({
  ...
  '&.active, &.active:hover': {
    ...
    backgroundImage: `linear-gradient(98deg, ${theme.palette.customColors.primaryGradient}, ${theme.palette.info.main} 94%)`
  },
...
const VerticalNavLink = ({
  ...
  const activeItem = [item.path].includes('/apps/invoice/list')
      ...
      >
        <Link passHref href={item.path === undefined ? '/' : `${item.path}`}>
          <MenuNavLink
            ...
            className= {
              (router.pathname === '/apps/invoice/preview/[id]')
              ?
                (activeItem === true) ? 'active' : ''
              :
