...
import CompanyLogo from 'src/@core/layouts/components/vertical/navigation/CompanyLogo'
  ...
  const menuHeaderPaddingLeft = () => {
    if (navCollapsed && !navHover) {
      if (userVerticalNavMenuBranding) {
        ...
        return (collapsedNavWidth - navigationBorderWidth - 30)
      }
    } else {
      return 6
    }
    ...
    <MenuHeaderWrapper className='nav-header' sx={{ pl: menuHeaderPaddingLeft() }}>
      {userVerticalNavMenuBranding ? (
        ...
        <Link href='/dashboards/ecommerce' passHref>
            <CompanyLogo height='55px' weight='130px' position='fixed'/>
        </Link>
      )}
