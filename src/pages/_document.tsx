...
const fbImagePath: string = 'public/images/logos/facebook-logo-250x250.svg'
...
class MyDocument extends Document<MyDocumentProps> {
  ...
  static async getInitialProps(ctx: DocumentContext): Promise<MyDocumentProps> {
    ...
    const combinedStyles: ReactElement[] = [
      ...Children.toArray(initialProps.styles).filter((child): child is ReactElement => React.isValidElement(child)),
      ...emotionStyleTags
    ]
  ...
  render(): ReactElement {
    ...
    const fbPath = path.resolve(process.cwd(), fbImagePath)
    const fbSprite = fs.readFileSync(fbPath, 'utf-8')
    return (
      <Html>
        <Head>
        ...
        <body>
          <div dangerouslySetInnerHTML={{ __html: fbSprite }} style={{ display: 'none' }} />
          <Main />
          <NextScript />
        </body>
      </Html>
