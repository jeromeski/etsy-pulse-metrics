...
const EcommerceTotalClients = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      ...
      const endPoint = `${serverUrl}/api/users/user-type/client`
      axios
        .get(endPoint)
        .then(res => {
          setTotalClients(res.data.data.length)
        })
    ...
    <Card sx={{ position: 'relative', overflow: 'visible', marginTop: { xs: 7.5 } }}>
      <CardContent>
        <Grid container spacing={6}>
          ...
          <Grid item xs={6}>
            <Img alt='Total Partners' src='/images/cards/pose_m18.png' />
          </Grid>
        </Grid>
      </CardContent>
    </Card>
...
export default EcommerceTotalClients
